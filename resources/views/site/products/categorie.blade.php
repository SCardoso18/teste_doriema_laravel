...
@section('content')
@include('site.master.includes.breadcrumb', ['titleBreadcrumb' => "{$categorie->description}"])
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div id="store" class="col-md-9">
                ...
                <form action="{{route('products.categorie.personalizedQuery')}}" method="POST">
                    @csrf
                    ...
                    <div id="aside" class="col-md-3">
                        ...
                        <div class="aside">
                            ...
                            <div class="checkbox-filter">
                                @foreach ($subcategoriesCategorie as $subcategorieCategorie)
                                <div class="input-checkbox">
                                    <input type="checkbox" id="subcategorie-{{$subcategorieCategorie->id}}" name="subcategorie[]" value=" {{$subcategorieCategorie->id}} "

                                    @if(isset($filters['subcategorie']))
                                    @foreach ($filters['subcategorie'] as $item)
                                    @if($item == $subcategorieCategorie->id)
                                    checked
                                    @endif
                                    @endforeach
                                    @endif
                                    ...
                                    >
                                    <label for="subcategorie-{{$subcategorieCategorie->id}}">
                                        <span></span>
                                        {{$subcategorieCategorie->name}}
                                    </label>
                                </div>
                                @endforeach
                            </div>
                        ...
                        <div class="aside">
                            ...
                            <div class="checkbox-filter">
                                @foreach ($brandsCategorie as $brandCategorie)
                                <div class="input-checkbox">
                                    <input type="checkbox" id="brand-{{$brandCategorie->id}}" name="brand[]" value=" {{$brandCategorie->id}} "

                                    @if(isset($filters['brand']))
                                    @foreach ($filters['brand'] as $item)
                                    @if($item == $brandCategorie->id)
                                    checked
                                    @endif
                                    @endforeach
                                    @endif
                                    ...
                                    <label for="brand-{{$brandCategorie->id}}">
                                        <span></span>
                                        {{$brandCategorie->name}}
                                    </label>
                                </div>
